              ...
              <div class="dropdown drp-user">
                ...
                <div class="dropdown-menu dropdown-menu-right profile-notification">
                  ...
                  <ul class="pro-body">
                    ...
                    <li><a href="{{ url('messages') }}" class="dropdown-item"><i class="feather icon-mail"></i> My Messages</a></li>
...
<!-- [ Main Content ] start -->
<section class="pcoded-main-container" style="margin-left:0px;">
                                    ...
                                    <div class="col-sm-12">
                                        <table class="table table-responsive invoice-table table-borderless p-l-20">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{ $company->address }}, {{ $company->city }}, {{ $company->state }} {{ $company->country }}</td>
                                                </tr>
                                                <tr>
                                                    <td><a class="text-secondary" href="mailto:demo@gmail.com" target="_top">{{ $company->email }}</a></td>
                                                </tr>
                                                <tr>
                                                    <td>{{ $company->phone }}</td>
                                                </tr>
                                ...
                                <div class="col-md-12 form-group">
                                  @if($invoice->status =="accepted")
                                    <form >
                                      ...
                                      <button class="btn btn-success m-b-10" type="button" onclick="payWithPaystack()"> Pay Now </button> 
                                    </form>
                                  @endif
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div class="row invoive-info">
                                <div class="col-md-4 col-xs-12 invoice-client-info">
                                    ...
                                    <p class="m-0 m-t-10">{{$client->address }}</p>
                                    <p class="m-0">{{$client->tel }}</p>
                                    <p><a class="text-secondary" href="mailto:demo@gmail.com" target="_top">{{$client->email }}</a></p>
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table invoice-detail-table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <h6>{{ ($inv_item->type == 'product' || $inv_item->type == 'service') ? $inv_item->item->name : $inv_item->description }}</h6>
                                                    </td>
                            ...
                            <div class="row">
                              <p>Note: if payment is made via bank, please endeavour to send the transaction details to {{ $company->email }} or call {{ $company->phone }} to confirm payments.</p>
                            </div>
                        </div>
                    </div>
                      ...
                      <form>
                        <div class="col-md-6">
                          <div class="form-group">
                            <label>Message</label>
                            <textarea id="revision-text" class="form-control" name="message"></textarea>
                          </div>
                          <button type="submit" onclick="save_review()" id="review-button" class="btn btn-primary">Review Qoute</button>
                        </div>
                      </form>
                    </div>
                  </div>
...
<script>
    ...
    const save_review = () => {
      let text = document.getElementById('revision-text').value;
      if(text.length < 1)
      {
        return alert('please add details to the text box');
      }
      let reviewButton = document.getElementById('review-button');
      reviewButton.disabled = true
      reviewButton.innerHTML = 'Sending...';
      let data = {
        'review': text,
      ...
      $.ajax({
          type: "POST",
          url: `{{ url('api/qoute-review/create') }}`,
          data :data,
          ...
          success: function (data) {
            ...
            text: 'Review has been submited',
            type: 'success'
        });
            reviewButton.innerHTML = 'Review Submitted';
            setTimeout(function() {
              reviewButton.disabled = false;
              document.getElementById('revision-text').value = '';
              reviewButton.innerHTML = 'Review Qoute';
...
<script>
  function payWithPaystack(){
    ...
    let koboAmount = parseInt(amount * 100)
    console.log(koboAmount)
    var handler = PaystackPop.setup({
      key: 'pk_test_5344780dd2026d1898008b4dd92c9ea46e9314a7',
      ...
      callback: function(response){
        if(response.status == 'success')
        {
          ...
          let data = {
            ...
            amount: amount,
            channel: 'online-stk',
            refrence_no: response.reference,
            status: 'success'

          }
          $.ajax({
            type: "POST",
            url: `{{ url('api/invoice-reciept/create') }}`,
            data :data,
            dataType: 'json',// dataType: "html",
            success: function (data) {
              console.log(response)
