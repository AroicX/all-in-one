            ...
            <div class="col-md-10">
                <table class="table table-hover">
                    <thead>
                        ...
                        <center>
                        <td class="" style="font-weight: 600; float:right;">Employee Name: {{ $user_details->name }}</td>
                        </center> 
                    ...
                    <tr style="width: 100%;">
                        ...
                        <td class="text-right" style="font-weight: 600; ">Designation:  {{ $designation->name }}</td> 
                        <td  class="text-right" style="font-weight: 600;">Branch: {{ $branch->name }}</td>
                        <td></td>
                    </tr>
                    <tr style="width: 100%;">
                        <td class="text-left" style="font-weight: 600;">Date of Birth: {{ $employee->date_of_birth }}</td>
                        <td class="text-right" style="font-weight: 600;">Date of Joining:  {{ $profile->join_date }}</td> 
                        <td class="text-right" style="font-weight: 600;"> Bank Name: {{ $salary->bank_name }}</td>
                        <td></td>
                    </tr>
                <tr style="width: 100%;">
                        <td class="text-left" style="font-weight: 600;">Bank A/c No: {{ $salary->bank_acc_no }}</td>
                        <td class="" style="font-weight: 600;"></td> 
                        <td></td>
                        <td></td>
                    </tr>
                <tr style="width: 100%; font-weight: 600;">
                        ...
                        <td><center><p style="float:left; ">Weekoff: {{ $week_off }}</p></center></td>
                    </tr>
                    <tr>
                    <td></td>
                    <td></td>
...
<table class="table table-bordered" style="border: 2px solid black !important;">
  ...
  <tr>
    ...
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($grade->basic_salary) }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($grade->basic_salary) }}</td>
  ...
  <tr>
    ...
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($value_custom_addition->value) }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($value_custom_addition->value) }}</td>
  </tr>
  <?php $total = $total + $value_custom_addition->value; } ?>
   <?php $total_staff_fees = 0; foreach($staff_fees as $key => $staff_fee){ if($staff_fee->addition == "1"){  ?>
  <tr>
    <td style="border:0px; border-left:2px solid black !important;">{{ $staff_fee->description }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($staff_fee->amount) }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($staff_fee->amount) }}</td>
  </tr>
  <?php $total_staff_fees = $total_staff_fees + $staff_fee->amount;} } ?>
  <tr>
...
<?php $a = $grade->basic_salary + $total + $total_staff_fees; ?>
</table>
</div>
<div class="col-md-6" style="margin:0px !important; padding:0px !important;">
<table class="table table-bordered" style="border: 2px solid black !important; ">
  ...
  <?php $totalll = 0; foreach($value_custom_subtractions as $key => $value_custom_subtraction){ ?>
    ...
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($value_custom_subtraction->value) }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($value_custom_subtraction->value) }}</td>
    </tr>
    <?php $totalll = $totalll + $value_custom_subtraction->value; } ?>
    <tr>
    ...
    @endif
    <?php $_total_staff_fees = 0; foreach($staff_fees as $key => $staff_fee){ if($staff_fee->addition == "0"){  ?>
  <tr>
    <td style="border:0px; border-left:2px solid black !important;">{{ $staff_fee->description }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($staff_fee->amount) }}</td>
    <td style="border:0px; border-left:2px solid black !important;">{{ number_format($staff_fee->amount) }}</td>
  </tr>
  <?php $_total_staff_fees = $_total_staff_fees + $staff_fee->amount;} } ?>
    <tr>
    <td style="font-weight: 600; border: 2px solid black !important;">Total</td>
    <td style="border: 2px solid black !important;">{{ number_format($loan_amount_left + $totalll + ($PAYE / 12) + $NHF + $pension + $loan_amount_left) }}</td>
    <td style="border: 2px solid black !important;">{{ $b = number_format($loan_amount_left + $totalll + ($PAYE / 12) + $NHF + $pension + $loan_amount_left) }}</td>
    </tr>
    <?php $b = $loan_amount_left + $totalll + ($PAYE / 12) + $NHF + $pension + $loan_amount_left + $_total_staff_fees; ?>
</table>
</div>
</div>
<?php
...
?>
<div class="col-md-12" style="border:0px; font-weight: 600; text-align: center; border-top: 2px solid black !important; border-bottom: 2px solid black !important;">
